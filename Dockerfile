...
RUN apt-get update && \
    ...
    apt-get install -y --no-install-recommends luajit libluajit-5.1-dev libliquid-dev libvolk-dev libfftw3-dev && \
...
RUN git clone --branch v0.11.0 https://github.com/vsergeev/luaradio.git /luaradio_src && \
    cd /luaradio_src/embed && \
    DESTDIR=/luaradio make install-lmod
...
ENV DEBIAN_FRONTEND=noninteractive
RUN apt-get update && \
    apt-get install -y --no-install-recommends python3 python3-numpy luajit libliquid1 libvolk-dev libfftw3-bin libpq5 tzdata ca-certificates gettext libmysqlclient21 libjpeg8 libopenexr-3-1-30 libpng16-16t64 libfreetype6 && \
    apt-get autoremove -y && \
    apt-get clean all && \
    rm -rf /var/lib/apt/lists/
